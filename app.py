...
# Komplett bereinigtes System ohne fehleranfällige externe Web-Abfragen
html_frei_ki_app = """
...
if (!Recognition) {
    ...
    rec.onresult = (e) => {
        ...
        if (gehoertLower.includes("okay garmin") || gehoertLower.includes("ok garmin") || gehoertLower.includes("okay gar")) {
            ...
            let antwortText = "Ich habe verstanden: " + befehl;
            
            // Deine integrierten Befehle blitzschnell prüfen
            if (befehl.includes("hallo")) {
                antwortText = "Hallo Lukas! Wie kann ich dir heute helfen?";
            } else if (befehl.includes("schule")) {
                antwortText = "Die Hölle wurde erfolgreich lokalisiert.";
            } else if (befehl.includes("fick dich")) {
                antwortText = "Dich auch, mein Freund.";
            } else if (befehl.includes("lukas")) {
                antwortText = "Nein, nicht Lukas!";
            } else if (befehl.includes("beenden")) {
                antwortText = "Programm wird beendet.";
            } else if (befehl) {
                // Wenn du etwas anderes sagst, antwortet die integrierte Simulation
                antwortText = "Das ist eine gute Frage zu '" + befehl + "'. Ich bin als Garmin KI bereit!";
            }
            
            // Antwort im hellblauen Kasten anzeigen und sprechen
